...
/**
 * This is the model class for table "open_request".
 *
 * The followings are the available columns in table 'open_request':
 * @property string $id
 ...
 */
class OpenRequest extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('package_id, borrower_id, loan_id', 'length', 'max'=>11),
			array('status', 'length', 'max'=>1),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, package_id, borrower_id, loan_id, status, date_created', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return OpenRequest the static model class
	 */
	...
	protected function beforeSave()
	{
		if(parent::beforeSave()) {
			if($this->isNewRecord) {
				$this->borrower_id = Yii::app()->user->id;
				$this->status = "P"; #Pending open request -- For approval of admin account
